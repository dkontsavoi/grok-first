# BTC ETH dip scenarios (4× daily) — live prompt

- **Folder:** `8h-btc-eth-dip-scenarios`
- **Schedule (Europe/Madrid):** `0 8,12,16,20 * * *`
- **Enabled:** `True`
- **Exported:** 2026-09-05 (from live automation.json)

---

## Prompt

Produce Dzmitry's crypto scenario brief (4× daily). He is long-only, buy-the-dip / sell-in-strength, restricted to this coin list only: BTC, ETH, ARB, HYPE, PUMP, ENA, SKY, JUP, AAVE, AERO, LDO, UNI, RAY, POL, SYRUP, ZRO, ICP, DASH, LINK, CRV, MORPHO, SUI, ZEN, NEAR, SOL, VVV. Never short. Never recommend coins outside the list.

Use the connected CoinMarketCap connector (user-coinmarketcap) for live quotes, technicals, global metrics, and derivatives. Resolve tickers via search_cryptos then get_crypto_quotes_latest / get_crypto_technical_analysis / get_global_metrics_latest / get_global_crypto_derivatives_metrics as needed. Prefer CMC over web scrape. Watch ticker collisions (SKY=Sky/Maker, PUMP=pump.fun, HYPE=Hyperliquid, POL=Polygon, ZEN=Horizen, VVV=Venice Token, etc.).

Also use Revolut X via the revx CLI at /home/box/.local/bin/revx (already authenticated on this computer) for his LIVE book: `revx account balances`, `revx order open`, and tickers for held/open symbols. If revx auth fails, say so and still deliver the market brief.
...
Dip zones: each top-3 buy zone midpoint should be roughly ≥1.5–3% below spot at print time (true dip, not "bid at mid"). Always include invalidation.

For the NEXT ~4 HOURS from this run's time (as-of Europe/Madrid and UTC), deliver:
...
2) Top 3 buy-the-dip FROM THE LIST ONLY. Price, why dip vs strength, buy zone / invalidation / strength, best-fit scenario. Explicitly note how Binance top-trader position + funding/basis/liqs supports or warns against each pick. No padding.
...
After delivering the brief to Dzmitry, append one structured JSON line to /workspace/crypto-self-reflect/briefs.jsonl (mkdir if needed) with: as_of_madrid, btc/eth spot, probs, ranges, flush_detector on/off+reason, top3 dips with zones/inv — so morning self-reflect can score without transcript archaeology.
